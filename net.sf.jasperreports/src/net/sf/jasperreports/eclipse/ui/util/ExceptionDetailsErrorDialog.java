...
public class ExceptionDetailsErrorDialog extends IconAndMessageDialog {
	...
	public ExceptionDetailsErrorDialog(Shell parentShell, String dialogTitle, String message, IStatus status, int displayMask) {
		super(parentShell);
		this.title = buildTitle(dialogTitle);
		this.message = buildMessage(message, status);
		this.status = status;
	...
	public String buildMessage(String message, IStatus status) {
		String msg = message == null ? status.getMessage() : MessageFormat.format("{0}\n\nReason: {1}", new Object[] { message, status.getMessage() });
	...
	protected Control createDialogArea(Composite parent) {
		...
		GridDataFactory.fillDefaults().align(SWT.FILL, SWT.FILL).grab(true, true).hint(convertHorizontalDLUsToPixels(IDialogConstants.MINIMUM_MESSAGE_AREA_WIDTH), convertVerticalDLUsToPixels(100))
	 ...
	 *          The list to fill.
	 */
	protected void populateList(Text listToPopulate) {
